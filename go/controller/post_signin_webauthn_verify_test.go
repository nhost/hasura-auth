...
func TestPostSigninWebauthnVerify(t *testing.T) { //nolint:maintidx
	t.Parallel()
	...
	cases := []testRequest[api.PostSigninWebauthnVerifyRequestObject, api.PostSigninWebauthnVerifyResponseObject]{
		{
			name: "success",
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			resp := assertRequest(
				...
				cmpopts.IgnoreFields(
					api.PostSigninWebauthn200JSONResponse{}, //nolint:exhaustruct
					"Challenge",
				),
			)

			resp200, ok := resp.(api.PostSigninWebauthnVerify200JSONResponse)
			if ok {

...
var (
	ErrInvalidPhoneFormat = errors.New("phone number must be in international format (e.g., +64211234567)")
...
type modicaSMSRequest struct {
	...
	Source      string `json:"source,omitempty"`    // Optional short code
	Reference   string `json:"reference,omitempty"` // Optional alternative reference
	Class       string `json:"class,omitempty"`     // Optional message class
}
...
func (s *ModicaSMS) SendSMS(to string, body string) error {
	...
	reqBody := modicaSMSRequest{ //nolint:exhaustruct
		Destination: to,
		Content:     body,
	}
	...
	req, err := http.NewRequestWithContext(context.Background(), http.MethodPost, modicaAPIURL, bytes.NewBuffer(jsonData))
	if err != nil {
